...
npm run build        # production build → dist/  (Netlify base "/")
...
DEPLOY_TARGET=gh-pages npm run build        # GitHub Pages build (base "/cbt-flashcards/")
```
...
- **Rendering a sign** (`components/SignPlate.svelte`): resolves the bundled SVG via `import.meta.glob`; ~17 `composite` signs (road markings, traffic-light signals, worded direction panels) are drawn in-app by `SignComposite.svelte`; anything missing falls back to a generated `SignPlaceholder.svelte`. **A new composite id must get a branch in `SignComposite.svelte`** or it renders the "?" fallback.
...
- **Pure, framework-agnostic, unit-tested modules** (import these for logic; they have no DOM/window deps):
  ...
  - `deck.ts` — `activeDeck()` (the in-scope filter: `!excludeFromV1`, edge gated by `includeEdge`, markings by `includeMarkings`) and `buildStudyQueue()` (due reviews interleaved with new cards).
...
### Deploy
Deploy-ready for both targets: `vite.config.ts` switches `base` on `DEPLOY_TARGET`; `netlify.toml` (root) and `.github/workflows/deploy.yml` (Pages, Node 22) are committed. PWA (offline) is generated by `vite-plugin-pwa`.

## Conventions
- Keep `npm run check` and `npm test` green before considering a change done.
- Generated/derived files (`src/data/signs.ts`, `src/data/_gen/asset-manifest.json`, `dist/`, `public/icons/`) are outputs — change the generator, not the output.
