...
A static, client-only **UK motorcycle CBT road-signs revision app** (Svelte 5 + Vite + TypeScript, GSAP for motion). No backend; no tracking by default (optional, opt-in privacy-friendly analytics — see Deploy); all state in `localStorage`. Motorway signs are an **opt-in module** (the `motorway` category, gated by the `includeMotorway` setting — off by default, since motorways are beyond CBT scope), drawn mostly as in-app composites (a few have bundled SVGs).
...
- **`src/lib/store.svelte.ts`** is the single source of truth — a Svelte 5 `$state` rune object holding `reviews`/`settings`/`sessions`. It owns all mutations (`gradeRecall`/`gradeQuiz` → `gradeSign`, settings, reset, export/import, backup-nudge), prunes orphan reviews on load, and persists **debounced + flushed on `pagehide`/`visibilitychange`** (mobile-safe). `quizFocus` is a one-shot id list the Report sets for "Drill these".
- **Pure, framework-agnostic, unit-tested modules** (import these for logic; they have no DOM/window deps):
  ...
  - `deck.ts` — `activeDeck()` (the in-scope filter: tier gated by the **`deckScope`** slider via `SCOPE_TIERS` — essential/standard/comprehensive — with the `marking` and `motorway` categories as orthogonal opt-in toggles `includeMarkings`/`includeMotorway`) and `buildStudyQueue()` (due reviews interleaved with new cards; the `shuffleCategories` setting — passed as `buildStudyQueue`'s `shuffleNew` arg — mixes new cards across categories instead of importance order). Settings migrate the legacy `includeEdge` boolean → `deckScope`.
  ...
  - `persistence.ts` / `storage.ts` — load/save/sanitise the `localStorage` blob and request durable (non-evictable) storage; `backup.ts` — the pure backup-nudge heuristic (`shouldNudgeBackup`). `util.ts` — `todayStr`/`pct`/`clamp`/`humanInterval`.
- **`activeDeck()` vs the full set**: Study and Quiz use `activeSigns()` (respects settings); the Reference (`Browse.svelte`) deliberately shows the *complete* set regardless of the scope slider, but still honours the `includeMotorway` opt-in.
...
### Deploy
GitHub Pages via Actions: `.github/workflows/deploy.yml` (Node 22) builds with `DEPLOY_TARGET=gh-pages` and deploys on every push to `main`. `vite.config.ts` derives the Pages `base` from `GITHUB_REPOSITORY` (rename-proof); local dev/preview use `/`. PWA (offline) is generated by `vite-plugin-pwa`. **Analytics** are off by default: `src/lib/analytics.ts` (GoatCounter, cookieless) no-ops unless a `GOATCOUNTER` repo variable is set, which the workflow passes through to the build as `VITE_GOATCOUNTER`.
